...
{   
   ...
   // Used by PDFViewMain to identify User Action for Text Selectopn vdpTextMode
   ...
   // Used for VDP Text Sel to specify eitther Field Lenght is Fixed, or BBox is 
   ...
   public static enum GraphicsPaths { 
      ...
      {
         if (LINE_PATH.toString().equals(action)) return true;
         return false;
      }
      public static boolean isQuadPath(String action) 
      {
         if (QUAD_PATH.toString().equals(action)) return true;
         return false;
      }
      public static boolean isCubicPath(String action) 
      {
         if (CUBIC_PATH.toString().equals(action)) return true;
         return false;
      }
   } 
   ...
   //Mouse Actions, No UI Events are registered. It acts as indicator to Mouse Motion        
   public static enum MouseActions { 
      TRANSLATED, SCALED, ROTATED, PATH_ADJUSTED, CIRCLE_FILTER_ADJUSTED, 
      RADIAL_FILTER_ADJUSTED, FILTER_CIRCLE_ADJUSTED, GRID_POINT_ADJUSTED
   }    
