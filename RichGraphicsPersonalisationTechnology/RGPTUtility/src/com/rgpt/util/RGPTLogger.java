...
public class RGPTLogger {
	...
	public static int m_DefLogInd = INFO_LOG; // This indicator is used if Log
												// Level is not set
	public static int m_DefLog = DEBUG_LOG; // This is set if in the method call
	...
	// m_ConsoleLog is the logger for this class. This inherits all the
	// propoerties
	// of the Root Category. This is set to print in the console in the property
	// file
	...
	private static void populateLogger() {
		...
		if (prop == null)
			return;
		System.out.println("Populating the Logger");
		...
		if (appendLog == null)
			return;
		if (appendLog.equals("true"))
			m_AppendLog = true;
		else
			m_AppendLog = false;
		if (m_Logger == null || m_Logger.length() == 0 || m_Logger == "CONSOLE")
			return;
		if (m_Logger.equals("LOG4J")) {
			try {
				...
				if (m_LogLevel.length() > 0) {
					...
					if (logIndObj != null)
						m_LogInd = logIndObj.intValue();
					else
						m_LogInd = m_DefLogInd;
				} else
					m_LogInd = m_DefLogInd;
				return;
			}
			...
			while (appendars.hasMoreElements()) {
				...
				if (className.equals("org.apache.log4j.FileAppender")) {
					...
					String logFile = RGPTFileFilter.getFileName(fAppendar
							.getFile());
					...
					if (RGPTParams.m_RGPTParamValues != null) {
						m_LogDir = RGPTParams.getVal("LogDir", m_LogDir);
						m_AppendLog = RGPTParams.getBoolVal("AppendLog",
								m_AppendLog);
					}
			...
			// By default the Looger inherits all the property of the Root
			// Logger.
	...
	public static void logToFile(String logMesg, Throwable ex) {
		StringBuffer mesg = new StringBuffer(logMesg + " Exception Mesg: "
				+ ex.getMessage());
