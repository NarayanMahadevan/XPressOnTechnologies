...
public class XONImageDesigner extends JFrame implements RGPTActionListener,
	...
	public XONImageDesigner(boolean isWindowsApp, Dimension contentPaneSize) {
		...
		m_WestPanelWidth = AppletParameters.getIntVal("WestPanelWidth");
		m_SouthPanelHeight = AppletParameters.getIntVal("SouthPanelHeight");
	...
	private JPanel createCenterPane() throws Exception {
		...
		int panelHt = AppletParameters.getIntVal("IDAppsWFPanelHeight");
		int panelWt = m_ContentPaneSize.width - m_WestPanelWidth
				- AppletParameters.getIntVal("IDAppsEastPanelWidth");
	...
	private JPanel createEastPanel() {
		...
		int sthEastHt = AppletParameters
				.getIntVal("IDAppsSouthEastPanelHeight");
		int eastPanelWt = AppletParameters.getIntVal("IDAppsEastPanelWidth");
		...
		int wt = eastPanelWt, ht = AppletParameters.getIntVal("LabelHeight");
	...
	private JPanel createShapePreviewPanel() {
		...
		// Preview Pane
		int wt = AppletParameters.getIntVal("IDAppsPreviewPanelWidth");
		int labelHt = AppletParameters.getIntVal("LabelHeight");
		int zoomPanelHt = AppletParameters.getIntVal("IDAppsZoomPanelHeight");
		String lblTxt = lu.getText("ImagePreview"), fontSzProp = "LabelFontSize";
		...
		JPanel zoomPanel = RGPTUIUtil.createPanel(m_ZoomViewPane, true,
				RGPTUIUtil.LOWERED_BORDER);
		int zoomlHt = AppletParameters.getIntVal("IDAppsZoomViewHeight");
	...
	private JPanel createWestPanel() {
		...
		JPanel adsLabel = RGPTUIUtil.createLabelPanel(lblTxt, m_WestPanelWidth,
				AppletParameters.getIntVal("LabelHeight"), "LabelFontSize");
		...
		JPanel adsPanel = RGPTUIUtil.createPanel(m_AdsViewPane, true,
				RGPTUIUtil.LOWERED_BORDER);
		int adsHt = AppletParameters.getIntVal("IDAppsAdsPanelHeight");
	...
	private JPanel createSetShapPanel() {
		...
		int panelHt = AppletParameters.getIntVal("IDAppsWestPanelHeight");
		RGPTUIUtil.setCompSize(setShapePanel, m_WestPanelWidth, panelHt, 5);

		int wt = m_WestPanelWidth, ht = AppletParameters
				.getIntVal("LabelHeight");
	...
	private JPanel createFileActionPanel() {
		...
		int filePanelHt = AppletParameters
				.getIntVal("IDAppsFileActionPanelHeight");
		buttonPanel.setMinimumSize(new Dimension(m_WestPanelWidth - 20, 120));
	...
	private JPanel createZoomPanel() {
		LocalizationUtil lu = new LocalizationUtil();
		int westPanelWidth = AppletParameters.getIntVal("WestPanelWidth");
		int southPanelHeight = AppletParameters.getIntVal("SouthPanelHeight");
	...
	private JPanel createImageFilterPanel(String action, int vgap) {
		int panelWt = AppletParameters.getIntVal("IDAppsPreviewPanelWidth");
	...
	private boolean openFile(IDAppsActions action,
		...
		if (imgHolder != null) {
			try {
				int thumbImgWt = AppletParameters
						.getIntVal("ThumbviewImageWidth");
				int thumbImgHt = AppletParameters
						.getIntVal("ThumbviewImageHeight");
	...
	private void showImageFilterPreview(Graphics g) {
		...
		int vGap = AppletParameters.getIntVal("ThumbviewVerticalGap");
		...
		String[] imageFilters = AppletParameters.getVal(actionId).split("::");
	...
	private Rectangle drawImagePreview(Graphics2D g2d, BufferedImage thumbImg,
			...
			boolean setSel) {
		int frameMargin = AppletParameters.getIntVal("FrameMargin");
		int arc = AppletParameters.getIntVal("ThumbviewRoundRectArc");
	...
	public static void main(String[] args) {
		...
		AppletParameters.createServerProperties();
		Color colorpanel, buttonColor, fontColor;
		colorpanel = RGPTUtil.getColVal(AppletParameters.getVal("PANEL_COLOR"),
				":");
		buttonColor = RGPTUtil.getColVal(
				AppletParameters.getVal("BUTTON_COLOR"), ":");
		fontColor = RGPTUtil.getColVal(AppletParameters.getVal("FONT_COLOR"),
				":");
