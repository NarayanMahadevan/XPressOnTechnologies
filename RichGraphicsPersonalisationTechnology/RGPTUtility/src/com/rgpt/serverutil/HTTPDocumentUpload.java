...
import com.rgpt.util.RGPTProgressBar;
...
public class HTTPDocumentUpload implements DocumentUploadInterface,
	...
	static {
		...
		m_NotificationStatus.put(
				NotificationHandlerInterface.STATUS_IN_PROGRESS,
	...
	public int getFileUploadId(HashMap connInfo) throws Exception {
		...
		try {
			fileUpldInfoStream = makeFileUpldInfoServerCall(connInfo,
					"GET_FILE_UPLOAD_ID", -1);
			...
			// Once we are done we want to make sure to disconnect from the
			// server.
			if (dataStr != null)
				dataStr.close();
		}
	...
	public void registerNotificationHandler(
			NotificationHandlerInterface notifyHdlr) {
	...
	public HashMap uploadDocument(HashMap connInfo, ServerInterface serverReq,
		...
		return this.uploadDocument(connInfo, serverReq, fileUploadId, files,
				false);
	}

	public HashMap uploadDocument(HashMap connInfo, ServerInterface serverReq,
			int fileUploadId, Vector files, boolean upldPath) throws Exception {
		File f = null;
		ZipEntry entry = null;
		...
		try {
			...
			if (m_NotificationHandler != null)
				m_NotificationHandler.notifyProgress(srcName,
						NotificationHandlerInterface.STATUS_STARTED,
						new HashMap());
			...
			for (int i = 0; i < files.size(); i++) {
				...
				System.out.println("Number of Files Written into Stream: " + i
						+ 1 + " Individual File Size: " + indFileSize / 1000
						+ " Total File Size Written: " + totalFileSize / 1000
						+ " Total File Size is: " + totalFileLength / 1000);
				out.closeEntry();
			}

			// Once we are done writing out our stream we will finish building
			// the
			...
			// Now that we have set all the connection parameters and prepared
			// all
	...
	public void getFileUploadData(HashMap connInfo, int fileUploadId,
		...
		RGPTLogger.logToFile("Creatting Thread Request: "
	...
	public void processThreadRequest(HashMap requestData) throws Exception {
		...
		if (reqType.equals("GetFileUploadData"))
	...
	public void GetFileUploadData(HashMap requestData) throws Exception {
		...
		try {
			...
			int fileUpldId = ((Integer) requestData.get("FileUploadId"))
					.intValue();
			// Response Steam from the server
			RGPTLogger
					.logToFile("Making Server Call for file upload data for: "
							+ fileUpldId);
			fileUpldInfoStream = makeFileUpldInfoServerCall(connInfo,
					"GET_FILE_UPLOAD_INFO", fileUpldId);
			dataStr = new ObjectInputStream(fileUpldInfoStream);
			...
			if (fileUploadInfo.m_TransferRate > 0)
				progInfo.m_TimeLeft = (int) ((progInfo.m_TotalNumOfBytes / fileUploadInfo.m_TransferRate) - fileUploadInfo.m_TotalTimeTaken);
			...
			RGPTLogger.logToFile("File Upload Info Data: "
					+ fileUploadInfo.toString());
			if (isFinished) {
				String srcName = this.getClass().getSimpleName();
				progBar.dispose();
				if (m_NotificationHandler != null)
					m_NotificationHandler.notifyProgress(srcName,
							NotificationHandlerInterface.STATUS_FINISHED,
							new HashMap());
				return;
			}
			try {
	...
	private InputStream makeFileUpldInfoServerCall(HashMap connInfo,
			String reqName, int fileUpldId) {
		String urlStr = "", servName = "";
		urlStr = (String) connInfo.get("URLCodeBase");
	...
	private HttpURLConnection getHttpConnection(URL serverURL) throws Exception {
		HttpURLConnection conn = null;
		...
		System.out.println("URLConn Classname: "
				+ serverConn.getClass().toString());
