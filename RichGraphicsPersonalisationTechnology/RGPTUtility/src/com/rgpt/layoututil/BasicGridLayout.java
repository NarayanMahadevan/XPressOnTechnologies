...
/*
   This is a useful layout which is more powerful than the GridLayout. Like the GridLayout,
   you specify a number of rows and columns for your grid, but instead of making each 
   component the same size, it makes each column as wide as the widest component it it
   and each row as tall as the tallest component in it, It then uses separate row and column 
   weightings, (like GridBagLayout) to fill in any extra space. The default weights are 
   zero which puts all components at the top left, but if you set the default weights to 1, it 
   behaves like GridLayout. Finally, it applies an alignment and fill to each component in its 
   cell. The main use for this class is as a superclass for GridLayoutPlus (below), but it can
    be useful in its own right. There are a few methods which you can override to customize
    the behavior of this class for producing your own specialized grids.
